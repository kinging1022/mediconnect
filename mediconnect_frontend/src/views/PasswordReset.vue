  ...
  <div class="min-h-screen w-full bg-gradient-to-br from-blue-50 via-white to-blue-50 flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 relative overflow-hidden">
      <div v-if="formState === 'success'" class="text-center animate-fade-in">
        ...
        <h2 class="text-xl font-semibold text-gray-900 mb-2">Password Successfully Reset!</h2>
        <p class="text-gray-600 mb-6">Your password has been successfully updated. You can now sign in with your new password.</p>
        <RouterLink 
          to="/login" 
      ...
      <div v-else>
        ...
        <p class="mt-2 text-gray-600 text-center">Please enter your new password below</p>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          <div>
            ...
            <p v-if="password.length < 8" class="mt-2 text-sm text-red-600">Password must be at least 8 characters long.</p>
          </div>
          ...
          <div>
            ...
            <p v-if="confirmPassword && password !== confirmPassword" class="mt-2 text-sm text-red-600">Passwords do not match.</p>
          </div>
        ...
        <p v-if="errorMessage" class="mt-4 text-sm text-red-600 text-center animate-fade-in">{{ errorMessage }}</p>
      </div>
    </div>
  </div>
</template>
