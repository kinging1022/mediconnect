    ...
    <div class="min-h-screen w-full bg-gradient-to-br from-blue-50 via-white to-blue-50 flex items-center justify-center p-4">
      <div class="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 relative overflow-hidden">
        ...
        <div v-if="status === 'success'" class="text-center animate-fade-in">
          ...
          <button
            @click="status = 'idle'"
            class="text-blue-600 hover:text-blue-700 font-medium flex items-center justify-center mx-auto"
          >
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-6">
          <div>
            ...
            <p v-if="errorMessage" class="mt-1 text-sm text-red-500 animate-fade-in">
              {{ errorMessage }}
            </p>
          </div>
          <button
      ...
      async handleSubmit() {
        ...
        try {
          ...
          this.status = 'success';
            ...
            if(error && error.response.data && error.status === 400){
                if(error.response.data.error === 'your account is inactive , click the link on you email to activate your account'){
                    this.status = 'error';
                    this.errorMessage = error.response.data.error;
                    await this.sendActivationEmail(this.email)
                }else if(error.response.data.error === "User with this email does not exist"){
                    this.status = 'error';
                    this.errorMessage = error.response.data.error; 
                }
                ...
                this.status = 'error';
                this.errorMessage = 'Something went wront please try again'
            }
