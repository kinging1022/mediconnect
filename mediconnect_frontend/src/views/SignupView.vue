  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 py-12 px-4 sm:px-6 lg:px-8 flex items-center">
    <div class="max-w-md w-full mx-auto space-y-8 bg-white p-8 rounded-2xl shadow-lg">
      ...
      <div v-if="step === 2" class="space-y-6">
        <h3 class="text-lg font-medium text-gray-900">Create Password</h3>
        <div>
      ...
      <div class="flex justify-between mt-8">
        ...
        >
          <template v-if="step === 2">Create Account</template>
          <template v-else>
            Next
  ...
  methods: {
    ...
    validateStep() {
      ...
      if (this.step === 1) {
        ...
        if (!this.formData.password2) this.errors.password2 = 'Please confirm your password';
      ...
      if (this.validateStep()) {
        try {
          ...
          if (error.status === 400) {
            ...
            this.toastStore.showToast(5000, 'An error occurred while signing up, contact support', 'bg-red-500 text-white');
          }
        }
      }
    },
